// test-report-generator.js
  ...
  function prepareScenarioResults(testLog, config = {}) {
    ...
    const scenarioStartRegex = /Running scenario: (.*)/i;
    ...
    try {
      ...
      // Also check for "Total scenarios: X" format
      const totalScenarioMatch = testLog.match(/Total scenarios: (\d+)/);
      if (totalScenarioMatch && parseInt(totalScenarioMatch[1]) > 0) {
        console.log(`Found indication of ${totalScenarioMatch[1]} scenarios`);
      }
      
      // Also add any scenario names we find in "Scenario X completed successfully" format
      const completedMatches = [...testLog.matchAll(new RegExp(simpleCompleteRegex, 'gi'))];
      for (const match of completedMatches) {
        if (match[1].includes('Scenario')) {
          const nameMatch = match[1].match(/Scenario "?([^"]*)"?/);
          if (nameMatch && nameMatch[1]) {
            scenarioNames.add(nameMatch[1].trim());
          }
        }
      }
    ...
    // Create scenarios based on the names we found
    for (const name of scenarioNames) {
      const isCompleted = testLog.includes(`Scenario "${name}" completed successfully`) || 
                          testLog.includes(`${name} completed successfully`);
    ...
    // Check if we have a total scenario count and need to add more
    const totalMatch = testLog.match(/Total scenarios: (\d+)/);
    if (totalMatch) {
      const totalScenarios = parseInt(totalMatch[1]);
      if (scenarios.length < totalScenarios) {
        // Add default scenarios to match expected count
        const defaultNames = [
  ...
  // Export the TestReportGenerator class and utility functions
module.exports = {
  TestReportGenerator,
